...
window.addEventListener('load', function() {
  ...
  var kinectronIpAddress = "172.16.216.148"; // FILL IN YOUR KINECTRON IP ADDRESS HERE
...
function createParticles() {
  ...
  for(var i = 0; i < numParticles; i++) {
    ...
    colors[i] = new THREE.Color(Math.abs(Math.random(255)), Math.abs(Math.random(255)), Math.abs(Math.random(255)));
  }

  // Add point cloud to scene
  particles.colors = colors;
  var material = new THREE.PointsMaterial( { size: 10, vertexColors: THREE.VertexColors, transparent: true } );
